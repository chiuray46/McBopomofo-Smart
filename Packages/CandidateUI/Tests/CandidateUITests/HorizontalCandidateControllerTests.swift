...
final class HorizontalCandidateControllerTests {
    ...
    @Test("Candidate window remains available while the input method is inactive")
    func testCandidateWindowPresentationPolicy() {
        let controller = HorizontalCandidateController()
        let window = controller.window
        #expect(window?.hidesOnDeactivate == false)
        #expect((window?.level.rawValue ?? 0) > NSWindow.Level.popUpMenu.rawValue)
